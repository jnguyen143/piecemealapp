...
import { encryptData } from './EncryptedRequests.js'; // eslint-disable-line import/extensions
...
function loadFriendsEvents() {
  ...
  for (const btn of document.getElementsByClassName('friend-delete')) {
    btn.addEventListener('click', (event) => {
      const userId = event.target.getAttribute('friend-id');
  ...
  // eslint-disable-next-line no-restricted-syntax
  for (const btn of document.getElementsByClassName('friend-request-accept')) {
    const listElement = btn.parentElement;
    btn.addEventListener('click', (event) => {
      const userId = event.target.getAttribute('friend-id');
      fetch('/api/handle-friend-request', {
        ...
        if (response.result !== 0) { showToast('Failed to add friend'); } else {
          listElement.remove();
  ...
  // eslint-disable-next-line no-restricted-syntax
  for (const btn of document.getElementsByClassName('friend-request-deny')) {
    const listElement = btn.parentElement;
    btn.addEventListener('click', (event) => {
      const userId = event.target.getAttribute('friend-id');
      fetch('/api/handle-friend-request', {
        ...
        if (response.result !== 0) { showToast('Failed to deny friend request'); } else {
          listElement.remove();
  ...
  document.getElementById('friends-add-search-button').addEventListener('click', () => {
    const nameField = document.getElementById('friends-add-name-field');
    const usernameField = document.getElementById('friends-add-username-field');
...
function loadFriendSearchEvents() {
  // eslint-disable-next-line no-restricted-syntax
  for (const btn of document.getElementsByClassName('search-results-user-add')) {
    btn.addEventListener('click', (event) => {
...
function loadProfileEvents() {
  ...
  document.getElementById('profile-new-username-button').onclick = () => {
    const newUsername = document.getElementById('profile-new-username-field').value;
  ...
  const oldPasswordField = document.getElementById('profile-old-password-field');
  if (oldPasswordField != null) {
    oldPasswordField.addEventListener('input', (event) => {
      ...
      if (value !== '') {
        ...
        const newPasswordField = document.getElementById('profile-new-password-field');
        if (newPasswordField.value !== '') { document.getElementById('profile-new-password-button').disabled = false; } else { document.getElementById('profile-new-password-button').disabled = true; }
      } else {
    ...
    document.getElementById('profile-new-password-field').addEventListener('input', (event) => {
      ...
      if (value !== '') {
        ...
        if (oldPasswordField.value !== '') { document.getElementById('profile-new-password-button').disabled = false; } else { document.getElementById('profile-new-password-button').disabled = true; }
      } else {
        document.getElementById('profile-new-password-button').disabled = true;
      }
    });
    ...
    document.getElementById('profile-new-password-button').onclick = () => {
      encryptData(JSON.stringify({
        ...
        fetch('/api/update-password', {
          ...
          if (response.result === 2) { showToast('Incorrect password'); } else if (response.result !== 0) { showToast('Failed to update password'); } else {
